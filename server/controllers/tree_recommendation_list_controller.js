...
exports.getTreeRecommendations = async (req, res) => {
  ...
  try {
    ...
    const sunhours = Math.round((sunhoursData[0].dataValues.sunhours_avg * 12) / 365 * 10) / 10;

    const precipitationData = await RegionalAverageMonthlyPrecipitationGermany.findAll({
      attributes: [
    ...
    const precipitation = Math.round(precipitationData[0].dataValues.precipitation_mm_avg * 10) / 10;
