...
function MapModule(props) {
  ...
  const [sunhours, setSunhoursDaily] = useState(0);
  ...
  useEffect(() => {
    ...
    fetch(
      `${SERVER_URL}/dashboard/regional_sunhours_single/${props.location_id}`
    )
      .then((res) => res.json())
      .then((data) => {
        setSunhoursDaily(data.sunhours);
      });
  }, [props.location_id]);
  ...
  return (
    ...
    >
      ...
      <Marker
        ...
        color="#3F6659"
      ></Marker>
      <MapOverlay elevation={props.elevation} sunhours={sunhours} />
    </Map>
  );
}
